...
type HeightTuple = [number, HeightUnits];

type WeightTuple = [number, WeightUnits];
...
class Person {
  ...
  set setSurname(name: string) {
    this.surname = name;
  }

  set setAge(age: number) {
    if (age < 1) throw new Error('Age must be greater than 0');
    if (age > 150) throw new Error('Age must not exceed 150');
    this.age = age;
  }

  set setHeight([height, units]: HeightTuple) {
    switch (units) {
      case HeightUnits.M:
  ...
  set setWeight([weight, units]: WeightTuple) {
    switch (units) {
